-- Recria função de status das funções de geração de código (caso ausente)
-- Data: 2025-08-31 20:20
begin;
...
DECLARE v jsonb; BEGIN
  ...
  RETURN jsonb_build_object('functions', COALESCE(v,'[]'::jsonb), 'now', now());
END;$$;
